...
const PropertyDataEntry = ({ form, setForm, platforms, setPlatforms }) => {
  ...
  const platformCategories = {
    ...
    'Browser Extensions': {
      platforms: ['Chrome Extension', 'Firefox Extension', 'Opera Extension', 'Safari Extension'],
      icon: '🔧',
  ...
  const propertyTypes = {
    'Website': { needsUsername: false, urlLabel: 'Website URL', placeholder: 'https://example.com' },
    'Instagram': { needsUsername: true, urlLabel: 'Profile URL', placeholder: 'https://instagram.com/username' },
    'TikTok': { needsUsername: true, urlLabel: 'Profile URL', placeholder: 'https://tiktok.com/@username' },
    'Facebook': { needsUsername: true, urlLabel: 'Profile URL', placeholder: 'https://facebook.com/username' },
    'YouTube': { needsUsername: true, urlLabel: 'Channel URL', placeholder: 'https://youtube.com/@username' },
    'Discord': { needsUsername: true, urlLabel: 'Server URL', placeholder: 'https://discord.gg/invite' },
    'Telegram': { needsUsername: true, urlLabel: 'Channel URL', placeholder: 'https://t.me/channel' },
    'Email': { needsUsername: false, urlLabel: 'Email Address', placeholder: 'contact@example.com' },
    'iOS App': { needsUsername: false, urlLabel: 'App Store URL', placeholder: 'https://apps.apple.com/app/...' },
    'Android App': { needsUsername: false, urlLabel: 'Play Store URL', placeholder: 'https://play.google.com/store/apps/...' },
    'Chrome Extension': { needsUsername: false, urlLabel: 'Extension URL', placeholder: 'https://chrome.google.com/webstore/...' },
    'Firefox Extension': { needsUsername: false, urlLabel: 'Extension URL', placeholder: 'https://addons.mozilla.org/...' },
    'Opera Extension': { needsUsername: false, urlLabel: 'Extension URL', placeholder: 'https://addons.opera.com/...' },
    'Safari Extension': { needsUsername: false, urlLabel: 'Extension URL', placeholder: 'https://apps.apple.com/...' }
  };
  ...
  useEffect(() => {
    ...
    if (form.platforms && Array.isArray(form.platforms) && form.platforms.length > 0) {
      ...
      form.platforms.forEach(platformData => {
        const config = propertyTypes[platformData.name] || platformData.config || { 
          needsUsername: true, 
  ...
  const handlePlatformCheckbox = (platformName) => {
    setPlatforms(prevPlatforms => {
      const existingPlatform = prevPlatforms.find(p => p.name === platformName);
  ...
  const addCustomPlatform = () => {
    if (newPlatform.trim() && !getAllDefaultPlatforms().includes(newPlatform.trim())) {
  ...
  const getPlatformData = (platformName) => {
    return platforms.find(p => p.name === platformName) || {
      url: '',
      username: '',
      config: propertyTypes[platformName] || { needsUsername: true, urlLabel: 'Profile URL', placeholder: 'https://example.com' }
    };
  };
  ...
  const renderPlatformCard = (platformName, isCustom = false) => {
    ...
    return (
      <div key={platformName} className={`card mb-3 ${isSelected ? 'border-primary' : ''}`}>
          ...
          <div className="d-flex justify-content-between align-items-center mb-3">
            <div className="form-check">
              <input
                ...
                checked={isSelected}
              />
              <label className="form-check-label fw-bold" htmlFor={`platform-${platformName}`}>
                {platformName}
              </label>
            </div>
            {isCustom && (
                ...
                <div className="col-12 col-md-6 mb-3">
                  <div className="form-group custom-input">
                    ...
                    <input
                      ...
                      onChange={(e) => handleInputChange(platformName, 'url', e.target.value)}
                      required
                    />
                  </div>
                </div>
                  ...
                  <div className="col-12 col-md-6 mb-3">
                    <div className="form-group custom-input">
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange(platformName, 'username', e.target.value)}
                        required
                      />
                    </div>
                  </div>
      ...
      <div className="card">
        <div className="card-header bg-primary text-white">
          <div className="main_title_head">
            ...
            <p className="mb-0 text-light">Select and configure your social media and digital platforms</p>
            {getSelectedCount() > 0 && (
              <div className="mt-2">
        ...
        <div className="card-body" style={{maxHeight: '600px', overflowY: 'auto'}}>
          {/* Platform Categories */}
          {Object.entries(platformCategories).map(([categoryName, categoryData]) => (
            <div key={categoryName} className="mb-4">
              <button
                type="button"
                onClick={() => toggleCategory(categoryName)}
                className="btn btn-light w-100 d-flex justify-content-between align-items-center p-3 mb-3"
              >
                <div className="d-flex align-items-center">
                  <span className="me-2" style={{fontSize: '1.2rem'}}>{categoryData.icon}</span>
                  <div className="text-start">
                    <div className="fw-bold">{categoryName}</div>
                    <small className="text-muted">{categoryData.description}</small>
                  </div>
                </div>
                {expandedCategories[categoryName] ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
              </button>
              
              {expandedCategories[categoryName] && (
                <div className="ps-3">
                  {categoryData.platforms.map(platform => renderPlatformCard(platform))}
                </div>
              )}
            </div>
          ))}
          ...
          <div className="mb-4">
            ...
            >
              <div className="d-flex align-items-center">
                ...
                <div className="text-start">
                  <div className="fw-bold">Custom Platforms</div>
                  <small className="text-muted">Add your own custom platforms</small>
                </div>
              </div>
              {expandedCategories['Custom Platforms'] ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </button>
            
            {expandedCategories['Custom Platforms'] && (
              <div className="ps-3">
                {/* Add Custom Platform */}
                <div className="card border-success mb-3" style={{backgroundColor: '#f8fff9'}}>
                  <div className="card-body p-3">
                    <div className="row">
                ...
                {getCustomPlatforms().length === 0 && (
                  <div className="text-center py-4 text-muted">
                    ...
                    <small>Add platforms that aren't in the standard categories above</small>
                  </div>
                )}
              </div>
            )}
