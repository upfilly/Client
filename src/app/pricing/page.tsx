...
export default function Pricing() {
  ...
  const getData = (p = {}) => {
    ...
    if (user) {
      let filter = { ...filters, ...p, userId: user?.id }
      let url = 'subscription-plan/all'
      ApiClient.get(url, filter).then(res => {
        if (res) {
          setData(res?.data?.data)
              ...
              <div className='row'>
                <div className='col-md-4'>
                  <div className='card set-bg p-4'>
                    ...
                    <p className='types'>Lorem Ipsum is simply dummy text of the printing and typesetting industry.
                    ...
                    <div className='d-flex align-items-center'>
                      <img src='/assets/img/person.jpg' className='icon_user' alt="" />
                      <div>
                        <h3 className='person-name ml-3'>Person Name</h3>
                        <p className='city-locatio ml-3'>City Location</p>
                      </div>
                    </div>
                  </div>
                </div>
